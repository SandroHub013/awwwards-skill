/* isolate canvas cost: sweep with full-size canvas vs 8px canvas */
...
for (const shrink of [false, true, false]) {
  const page = await browser.newPage();
  await page.setViewport({ width: 1440, height: 900 });
  const cdp = await page.createCDPSession();
  await cdp.send("Emulation.setCPUThrottlingRate", { rate: 4 });
  await page.goto(URL_, { waitUntil: "networkidle0" });
  if (shrink) await page.evaluate(() => { const c = document.querySelector("[data-land]"); c.width = 8; c.height = 8; });
  ...
  const res = await page.evaluate(() => new Promise((done) => {
    ...
    (function frame(t) {
      ...
      if (p < 1) requestAnimationFrame(frame);
        ...
        deltas.sort((a, b) => a - b);
        done({ n: deltas.length, median: deltas[deltas.length >> 1], p95: deltas[Math.floor(deltas.length * .95)] });
      }
    })(last);
  }));
  console.log(shrink ? "no-canvas:" : "full:     ", JSON.stringify(res));
  await page.close();
}
await browser.close();
server.close();
